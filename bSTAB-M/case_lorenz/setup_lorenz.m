function  [props] = setup_lorenz(props)
...
props.model.odeFun = @ode_lorenz; % a function handle @my_ode_function

% 1.2 define the state space dimension D
props.model.dof = 3; % degrees-of-freedom (= length of state vector)
...
props.model.odeParams = [sigma, r, b]; 
...
props.roi.N = 20000; % integer number

% 2.2 minima (per state space dimension)
props.roi.minLimits = [-10, -20, 0]; % must be of length <props.model.dof>

% 2.3 maxima (per state space dimension)
props.roi.maxLimits = [10, 20, 0]; % must be of length <props.model.dof>
...
props.bs.samplingVarDims = [true; true; false]; % boolean: dims to vary
...
options = odeset('RelTol',1e-8, 'AbsTol', 1e-6, 'Events', @lorenzStopFcn);
...
props.clust.featExtractFun = @features_lorenz; % a function handle
...
props.templ.Y0{1} = [0.8, -3.0, 0.0];   % initial condition (NOT the steady-state itself)
props.templ.modelParams{1} = [0.12, 0, -0.6]; % model parameters
props.templ.label{1} = 'butterfly1';   

% the initial condition that leads the system to end up on the second stable steady state
props.templ.Y0{2} = [-0.8, 3.0, 0];
props.templ.modelParams{2} = [0.12, 0, -0.6];
props.templ.label{2} = 'butterfly2'; 

props.templ.Y0{3} = [10, 50, 0];
props.templ.modelParams{3} = [0.12, 0, -0.6];
props.templ.label{3} = 'unbounded'; 
...
% props.eval.ampFun = @; % a function handle
