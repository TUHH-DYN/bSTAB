function  [props] = setup_friction(props)
...
props.model.odeFun = @ode_friction; % a function handle @my_ode_function
...
props.roi.minLimits = [-2, 0]; % must be of length <props.model.dof>

% 2.3 maxima (per state space dimension)
props.roi.maxLimits = [2, 2]; % must be of length <props.model.dof>
...
props.templ.Y0{2} = [2, 2];
props.templ.modelParams{2} = [1.5];
props.templ.label{2} = 'LC'; 
