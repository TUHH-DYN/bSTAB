...
bs_vals = zeros(ap_iters, props.templates.num_solutions+1); 
err_vals = zeros(ap_iters,props.templates.num_solutions+1);
solution_names = [props.templates.label, {'NaN'}];
...
for i = 1:length(solution_names)
    varNames{i} = ['bs_',solution_names{i}];
    varNames{length(solution_names)+i} = ['err_',solution_names{i}];
end
